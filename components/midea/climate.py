...
CONF_IDF_VALUE = "indoor_fan"
CONF_ODF_VALUE = "outdoor_fan"
    ...
    .extend(
        {
            ...
            cv.Optional(CONF_EEV): sensor.sensor_schema(
                unit_of_measurement=UNIT_EMPTY,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_EMPTY,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional(CONF_COMPRESSOR_TARGET): sensor.sensor_schema(
            ...
            cv.Optional(CONF_COMPRESSOR_VALUE): sensor.sensor_schema(
                unit_of_measurement=UNIT_HERTZ,
                icon=ICON_ROTATE_RIGHT,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_FREQUENCY,
            ...
            cv.Optional(CONF_VAL2_12): sensor.sensor_schema(
                unit_of_measurement=UNIT_EMPTY,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_EMPTY,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional(CONF_IDF_TARGET): sensor.sensor_schema(
                unit_of_measurement=UNIT_REVOLUTIONS_PER_MINUTE,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_EMPTY,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional(CONF_IDF_VALUE): sensor.sensor_schema(
                unit_of_measurement=UNIT_REVOLUTIONS_PER_MINUTE,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_EMPTY,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional(CONF_ODF_VALUE): sensor.sensor_schema(
                unit_of_measurement=UNIT_REVOLUTIONS_PER_MINUTE,
                accuracy_decimals=0,
                device_class=DEVICE_CLASS_EMPTY,
                state_class=STATE_CLASS_MEASUREMENT,
...
async def to_code(config):
    ...
    if CONF_T1_TEMPERATURE in config:
        sens = await sensor.new_sensor(config[CONF_T1_TEMPERATURE])
        cg.add(var.set_t1_temperature_sensor(sens))
    if CONF_T2_TEMPERATURE in config:
        sens = await sensor.new_sensor(config[CONF_T2_TEMPERATURE])
        cg.add(var.set_t2_temperature_sensor(sens))
    if CONF_T3_TEMPERATURE in config:
        sens = await sensor.new_sensor(config[CONF_T3_TEMPERATURE])
        cg.add(var.set_t3_temperature_sensor(sens))
    if CONF_T4_TEMPERATURE in config:
        sens = await sensor.new_sensor(config[CONF_T4_TEMPERATURE])
        cg.add(var.set_t4_temperature_sensor(sens))
    if CONF_EEV in config:
        sens = await sensor.new_sensor(config[CONF_EEV])
        cg.add(var.set_eev_sensor(sens))
    if CONF_POWER_USAGE in config:
        sens = await sensor.new_sensor(config[CONF_POWER_USAGE])
        cg.add(var.set_power_sensor(sens))
    if CONF_ENERGY_USAGE in config:
        sens = await sensor.new_sensor(config[CONF_ENERGY_USAGE])
        cg.add(var.set_energy_sensor(sens))
    if CONF_HUMIDITY_SETPOINT in config:
        sens = await sensor.new_sensor(config[CONF_HUMIDITY_SETPOINT])
        cg.add(var.set_humidity_setpoint_sensor(sens))
    if CONF_COMPRESSOR_TARGET in config:
        sens = await sensor.new_sensor(config[CONF_COMPRESSOR_TARGET])
        cg.add(var.set_compressor_target_sensor(sens))
    if CONF_COMPRESSOR_VALUE in config:
        sens = await sensor.new_sensor(config[CONF_COMPRESSOR_VALUE])
        cg.add(var.set_compressor_value_sensor(sens))
    if CONF_IDF_TARGET in config:
        sens = await sensor.new_sensor(config[CONF_IDF_TARGET])
        cg.add(var.set_idFTarget_sensor(sens))
    if CONF_IDF_VALUE in config:
        sens = await sensor.new_sensor(config[CONF_IDF_VALUE])
        cg.add(var.set_idFVal_sensor(sens))
    if CONF_ODF_VALUE in config:
        sens = await sensor.new_sensor(config[CONF_ODF_VALUE])
        cg.add(var.set_odFVal_sensor(sens))
    ...
    cg.add_library("pgistrand/MideaUART", "1.1.9")
